...
# Ix.yy = Imag component of initial complex value (x.yy is float)
# B     = Switch to B&W Palette
for parm in sys.argv[1:] :
    m = re.match("([BMVRI])([0-9\.\-\+e]*)",parm,re.I)
    if m :
        if m[1].upper() == "M" :
            try :
                reqIter = int(m[2])
...
def exportImage(IWid :int, IHei :int) -> None :
    ...
    global IPALR, IPALG, IPALB, InitRe, InitIm
    ...
    for j in range(IHei) :
        print(str(j))
        for i in range(IWid) :
            Re = (Scr_X_R - Scr_X_L) * i / (IWid-1) + Scr_X_L
